...
import { FEED_TITLE, SITE } from '../consts';
import { WRITING, absolute, post as postPath } from './urls';
import { getPosts } from './writing';

/**
 * The feed itself, in `lib/` so the route in `pages/rss.xml.ts` stays a thin
 * re-export and the XML assembly lives next to the URL helpers it depends on.
 */
export const rssHandler: APIRoute = async (context) => {
